    ...
    public static async Task SeedAsync(FarmDbContext context)
    {
        ...
        if (!await context.Crops.AnyAsync())
        {
            ...
            var crops = new List<Crop>
            {
                ...
                new Crop
                {
                    ...
                    ExpectedHarvestDate = new DateTime(2025, 10, 1),
        ...
        if (!await context.PlantingSchedules.AnyAsync())
        {
            ...
            var schedules = new List<PlantingSchedule>
            {
                ...
                new PlantingSchedule
                {
                    ...
                    FieldId         = fields[1].FieldId,
                    ScheduledDate   = new DateTime(2025, 10, 1),
                    ExpectedYieldKg = 3800.00m,
                    Status          = "Scheduled",
                    Notes           = "Main Kharif rice harvest"
                },
                new PlantingSchedule
                {
        ...
        if (!await context.Harvests.AnyAsync())
        {
            ...
            if (completedSchedule != null)
            {
                ...
                await context.Harvests.AddRangeAsync(harvests);
                await context.SaveChangesAsync();
            }
        }
        ...
        if (!await context.PestIncidents.AnyAsync())
        {
            ...
            var incidents = new List<PestIncident>
            {
                new PestIncident
                {
                    CropId         = crops[0].CropId,
                    PestName       = "Aphids",
                    Description    = "Heavy aphid infestation on wheat leaves",
                ...
                new PestIncident
                {
                    ...
                    Description    = "Early signs of BPH detected in rice field",
                    ReportedDate   = DateTime.Now.AddDays(-10),
                    Status         = IncidentStatus.Monitoring,
                    TreatmentNotes = "Applied neem-based spray, monitoring weekly"
                },
                new PestIncident
                {
                    CropId         = crops[2].CropId,
                    PestName       = "Whitefly",
                    Description    = "Whitefly attack resolved after treatment",
                    ReportedDate   = DateTime.Now.AddDays(-20),
                    Status         = IncidentStatus.Resolved,
                    TreatmentNotes = "Sprayed insecticide, issue resolved"
                }
            };
        ...
        if (!await context.ResourceUsages.AnyAsync())
        {
            ...
            if (schedule != null && resource != null)
            {
                ...
                await context.ResourceUsages.AddRangeAsync(usages);
                await context.SaveChangesAsync();
            }
        }
