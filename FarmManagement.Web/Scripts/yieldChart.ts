...
declare class Chart {
    constructor(ctx: CanvasRenderingContext2D, config: Record<string, unknown>);
    destroy(): void;
    update(): void;
}
 ...
 */
function renderYieldChart(canvasId: string, records: HarvestRecord[]): void {
    const canvas: HTMLCanvasElement | null = document.getElementById(canvasId) as HTMLCanvasElement | null;
    ...
    const allDates: string[] = [...new Set(records.map((r: HarvestRecord) => r.date))];

    // Build one dataset per crop
    const crops: string[] = [...new Set(records.map((r: HarvestRecord) => r.crop))];

    const datasets: Record<string, unknown>[] = crops.map((cropName: string, i: number) => {
        const color: CropColor = cropColors[i % cropColors.length];

        const data: (number | null)[] = allDates.map((date: string): number | null => {
            const match: HarvestRecord | undefined = records.find(
                (r: HarvestRecord) => r.crop === cropName && r.date === date
            );
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: (item: { dataset: { label: string }; parsed: { y: number | null } }) =>
                            '  ' + item.dataset.label + ':  ' + (item.parsed.y ?? 0).toLocaleString() + ' kg'
                    }
                }
            },
            scales: {
